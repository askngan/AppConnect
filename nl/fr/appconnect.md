...
Selon les besoins de votre activité, vous pouvez exécuter deux types de ressources dans {{site.data.keyword.appconserviceshort}} pour connecter vos applications : les serveurs d'intégration et les flux. Pour exécuter vos solutions IBM Integration Bus ou App Connect Enterprise, vous devez télécharger une solution d'intégration dans un fichier BAR, puis l'exécuter dans un serveur d'intégration dans {{site.data.keyword.appconserviceshort}}. Vous pouvez créer des flux dans {{site.data.keyword.appconserviceshort}} pour connecter vos applications de manière à ce que si un événement se produit dans une application, un autre événement se produira dans une autre application.  Vous pouvez créer des flux événementiels et des flux pour API.   
...
Dans cette section, nous détaillons les caractéristiques et la terminologie propres à {{site.data.keyword.appconserviceshort}} :
...
Un flux pour API contient une demande, une ou plusieurs actions d'application cible et une réponse. La demande utilise un modèle que vous définissez pour demander la création, le remplacement ou l'extraction d'objets de données dans vos applications. Lorsque la demande est soumise, chaque application cible exécute son action. Le flux renvoie ensuite une réponse qui confirme que les actions ont réussi ou renvoie les données demandées.
...
En plus d'ajouter des applications à vos flux, vous pouvez également ajouter des noeuds sous l'onglet **Logique** afin de configurer le traitement des données. Vous pouvez par exemple utiliser le noeud If pour ajouter un traitement conditionnel (effectuer différentes actions en fonction des données que vous recevez). Voir [Ajout d'une logique conditionnelle à un flux ![Icône de lien externe](../../icons/launch-glyph.svg "Icône de lien externe")](https://developer.ibm.com/integration/docs/app-connect/tutorials-for-ibm-app-connect/adding-conditional-logic-flow/). D'autre part, vous pouvez utiliser le noeud **For each** lorsque vous voulez effectuer une action pour chaque enregistrement retourné par une action d'extraction. Voir [Extraction d'éléments de vos applications ![Icône de lien externe](../../icons/launch-glyph.svg "Icône de lien externe")](https://developer.ibm.com/integration/docs/app-connect/tutorials-for-ibm-app-connect/using-ibm-app-connect-retrieve-items-applications/).
...
Vos flux et serveurs d'intégration sont représentés sous forme de vignettes sur le tableau de bord d'App Connect. Les vignettes affichent des informations récapitulatives sur le flux, l'API ou le serveur d'intégration, par exemple si un flux est en cours d'exécution ou arrêté ou s'il a produit une erreur. Vous pouvez cliquer sur les icônes de coche et de point d'exclamation pour voir quand le flux s'est exécuté correctement pour la dernière fois ou passer en revue les erreurs. Cliquez sur les trois points ![L'icône des 3 points verticaux ouvre un menu qui permet de démarrer, arrêter, modifier ou supprimer le flux](images/Menu.jpg) pour ouvrir un menu afin de démarrer, arrêter, modifier ou supprimer vos ressources. Les flux doivent être arrêtés avant de pouvoir être édités.
...
Vous pouvez ajouter plusieurs types d'actions à vos flux. Les actions communes sont la création (create), l'extraction (retrieve), la mise à jour ou création (update or create), mais certaines applications ont des actions spécifiques. Par exemple, l'application Watson Personality Insights a une action appelée "Analyze personality". Vous trouverez une liste des actions prises en charge par les applications dans {{site.data.keyword.appconserviceshort}} en tapant le type d'action dans la zone de recherche se trouvant dans la page Applications :
...
Pour certaines applications, vous devrez peut-être fournir des informations supplémentaires lorsque vous ajoutez une action de création à un flux afin que votre flux sache où créer l'objet. Par exemple, si vous utilisez une application de gestion de projet comme Asana ou Trello, si vous créez une tâche ou une carte, vous devez spécifier le projet ou le tableau où vous souhaitez l'ajouter.

**Mise à jour ou création (update or create)**

L'action update or create modifie un enregistrement existant dans votre application cible s'il existe, mais crée l'enregistrement s'il n'existe pas. Elle est également connue comme une action upsert (mise à jour ou insertion).

Par exemple, imaginons que quelqu'un soumette un formulaire Wufoo avec un changement d'adresse. Si le contact est déjà dans votre système CRM, vous voulez mettre à jour son adresse, mais dans le cas contraire, vous voulez l'ajouter. Comme pour l'action d'extraction, lorsque vous choisissez une action pour mettre à jour les données dans l'une de vos applications, vous pouvez ajouter une ou plusieurs conditions pour vous assurer que vous mettez à jour les bonnes informations.

Si plusieurs enregistrements correspondent à vos critères dans votre système cible, une erreur s'affiche sur le tableau de bord pour le flux et le flux ne met à jour ou ne crée aucun enregistrement. Par exemple, vous avez peut-être plus d'un contact avec les mêmes prénom et nom de famille. Vous pouvez donc essayer de faire correspondre un contact en utilisant des données uniques, telles que son adresse e-mail.

Vous êtes susceptible de voir les codes d'état suivants en réponse à une action de mise à jour ou de création. 
-   200 Un enregistrement a été mis à jour
-   201 Un enregistrement a été créé

Vous pouvez utiliser ces codes de réponse plus tard dans votre flux. Vous souhaitez peut-être prendre des mesures différentes selon qu'un enregistrement a été mis à jour ou créé. Pour obtenir un exemple de définition d'actions basées sur des codes de réponse, consultez le tutoriel [Création d'un flux événementiel qui met à jour ou crée un contact dans Salesforce et met à jour Asana chaque fois que vous recevez un formulaire dans Wufoo ![Icône de lien externe](../../icons/launch-glyph.svg "Icône de lien externe")](https://developer.ibm.com/integration/docs/app-connect/tutorials-for-ibm-app-connect/creating-event-driven-flow-updates-creates-contact-salesforce-updates-asana-whenever-receive-form-wufoo/).

**Extraction (retrieve)**

L'action d'extraction récupère les informations d'une application pour que vous puissiez l'utiliser dans une autre application.
...
Pour plus d'informations, voir [Extraction d'éléments de vos applications ![Icône de lien externe](../../icons/launch-glyph.svg "Icône de lien externe")](https://developer.ibm.com/integration/docs/app-connect/tutorials-for-ibm-app-connect/using-ibm-app-connect-retrieve-items-applications/).

## Mappage des données
{: #transforms}

Lorsque vous avez créé un flux, ajouté vos applications et sélectionné les actions appropriées, vous devez spécifier les informations à transférer entre vos applications. Lorsque vous ajoutez une action à votre flux, vous pouvez voir dans l'éditeur de flux une liste des zones disponibles pour cette application. Vous pouvez renseigner ces zones avec les données de votre application source ou les actions précédentes du flux. 
...
Si vous cliquez sur l'une de ces zones, vous voyez s'afficher deux icônes : **Insérer une référence** ![Icône Insérer une référence](images/InsertRef.jpg) et **Appliquer une fonction** ![Icône Appliquer une fonction](images/Functions.jpg). Si vous cliquez sur **Insérer une référence**, vous voyez les données disponibles que vous pouvez mettre dans cette zone, provenant des applications précédentes dans le flux. L'exemple suivant montre que nous pouvons choisir des zones venant de l'application source de Wufoo ou d'une action précédente de Salesforce dans le flux. Nous pouvons également utiliser le code d'état de l'action update or create de Salesforce.

![Capture d'écran montrant les entrées disponibles pour un mappage de données](images/Inputs.jpg)

Dans l'exemple suivant, notre flux est déclenché lorsqu'un nouveau formulaire complété est réceptionné dans Wufoo. Nous voulons créer un contact dans Salesforce pour la personne qui a soumis le formulaire. Ainsi, lorsque nous ajoutons au flux notre action "Créer un contact" dans Salesforce, nous copions les détails de notre contact du formulaire Wufoo. Ici nous pouvons voir que pour le nom du contact dans Salesforce, nous sélectionnons le nom de l'auteur du formulaire dans Wufoo. On reconnaît à la couleur que la zone mappée provient de Wufoo :

![Capture d'écran montrant que la zone Nom de Wufoo est mappée à la zone Nom de Salesforce](images/Mapping.jpg)

Dans l'exemple suivant, nous ajoutons au flux une action "Créer un message" dans Slack après une action "Mettre à jour ou créer un contact" dans Salesforce. Nous voulons créer un message sur Slack pour dire quel code de réponse est reçu pour l'action de Salesforce : 

![Capture d'écran d'une action Créer un message de Slack qui mappe un code réponse](images/SlackSC.jpg)

Vous pouvez voir que dans la zone **Texte** de l'action Slack "Créer un message", nous tapons un message, puis mappons le code d'état de l'action "Mettre à jour ou créer un contact" de Salesforce.

Prenons un autre exemple pour montrer une façon différente de mapper des codes de réponse. Cette fois, nous ajoutons un noeud "If" après une action "Mettre à jour ou créer un contact" de Salesforce parce que nous voulons effectuer des actions différentes selon qu'un contact Salesforce existant est mis à jour ou qu'un nouveau contact est créé. Dans ce cas, le code de réponse "200" signifie que le contact est mis à jour. Cette branche du noeud "If" contient donc une action spécifique à un enregistrement mis à jour.

![Capture d'écran illustrant l'utilisation des codes de réponse dans un noeud If](images/IfSC.jpg)

L'icône **Appliquer une fonction** ![Appliquer une fonction](images/Functions.jpg) vous montre une liste des fonctions de transformation que vous pouvez utiliser pour personnaliser les données que vous transmettez par votre flux. Ces fonctions peuvent être aussi simples que la conversion d'une zone spécifique en majuscules ou minuscules, ou légèrement plus complexes, comme la recherche et le remplacement de modèles spécifiques dans les données. Elles peuvent également être suffisamment puissantes pour former des expressions régulières. Vous pouvez soit sélectionner la fonction souhaitée dans la liste, soit la saisir vous-même. La syntaxe des fonctions est JSONata, un langage de requête et de transformation simple. Pour plus d'informations, voir [http://jsonata.org ![Icône de lien externe](../../icons/launch-glyph.svg "Icône de lien externe")](http://jsonata.org).


## Fichiers BAR et serveurs d'intégration
{: #barfiles}

Un fichier BAR est un fichier compressé auquel vous ajoutez des ressources déployables dans IBM Integration Bus ou App Connect Enterprise.  Lorsque vous développez une solution d'intégration dans Integration Bus ou App Connect Enterprise, vous regroupez vos flux de messages et toutes les ressources qu'ils utilisent dans un fichier BAR. Vous déployez ensuite le fichier BAR sur un serveur d'intégration. Ce serveur peut être sur site ou dans {{site.data.keyword.appconserviceshort}}.  Vous pouvez exécuter vos solutions Integration Bus ou App Connect Enterprise dans App Connect, sans avoir à acquérir et à maintenir une infrastructure informatique.  Lorsque vous téléchargez un fichier BAR vers App Connect, un serveur d'intégration est créé pour exécuter le contenu du fichier BAR.  Vous pouvez configurer l'authentification de base et la connectivité sécurisée entre vos ressources dans le cloud et sur site (voir [Exécution de vos solutions Integration Bus dans App Connect ![Icône de lien externe](../../icons/launch-glyph.svg "Icône de lien externe")](https://developer.ibm.com/integration/docs/app-connect/tutorials-for-ibm-app-connect/running-your-ibm-integration-bus-solutions-in-ibm-app-connect-enterprise-beta-plan)).  
