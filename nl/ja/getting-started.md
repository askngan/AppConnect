...
1.  まだアプリケーションに接続していない場合は、**「アプリケーションに接続 (Connect to _application_)」**をクリックし、アプリケーションに対するログインの詳細情報を入力します。一部のアプリケーションでは、接続に関する他の詳細情報を入力する必要が生じる場合があります。特定のアプリケーションのための資格情報を見つけるのに支援が必要な場合は、[How-to guides for apps ![外部リンク・アイコン](../../icons/launch-glyph.svg "外部リンク・アイコン")](https://developer.ibm.com/integration/docs/app-connect/how-to-guides-for-apps/) を参照してください。
...
1. オプション: 以下のようにして、フローの働きを詳細化できます。
    ...
    * 取得アクションをフローに追加して、特定の基準に一致する項目を取得するか、指定された数のレコードを取得します。また、エラー処理を定義するか、または取得された項目を処理する方法を定義する取得アクションをフローに追加することもできます ([Using App Connect to retrieve items from your applications ![外部リンク・アイコン](../../icons/launch-glyph.svg "外部リンク・アイコン")](https://developer.ibm.com/integration/docs/app-connect/tutorials-for-ibm-app-connect/using-ibm-app-connect-retrieve-items-applications/) を参照)。
...
1. フローを構成したら、ヘッダー・バー上のオプション・メニューを開き、**「フローの開始 (Start flow)」**をクリックします。
...
クラウド・ベースのアプリケーション内のデータを使用するアプリケーションを開発者が作成できるようにする場合は、API を提供できます。API 用のフローを作成するには、以下の手順を実行します。
...
1. フロー内で**「応答」**をクリックして、操作の終了時に返す応答を定義します。ターゲット・アプリケーションから使用可能なフィールドをマップします。 
1. **「完了」**をクリックして、モデルに戻ります。
1. すべてのモデルと操作を定義したら、メニューから**「API の開始 (Start API)」**を選択して、API を開始します。 

API のフローが準備できました。{{site.data.keyword.appconserviceshort}} ダッシュボード上で、API 用のフローは API アイコンで識別されます。 このフローは、その他のフローと同じ方法で開始したり停止したりできます。 API を実行中に開くことはできますが、編集する場合はその前に停止する必要があります。
...
## IBM Integration Bus 統合ソリューションの実行

IBM Integration Bus または App Connect Enterprise で開発した統合ソリューションをクラウドにデプロイできますが、IT インフラストラクチャーを確保して保守する必要はありません。統合ソリューションを構成するすべての成果物が含まれる BAR ファイルをインポートしてから、そのコンテンツを App Connect 内の統合サーバー内で実行できます。BAR ファイルを App Connect にアップロードするには、IBM Integration Bus バージョン 10.0.0.4 以降か App Connect Enterprise がオンプレミスでインストール済みでなければなりません。
...
1. オプション: オンプレミス環境と App Connect の間でメッセージ・フロー処理を分割するには、呼び出し可能フローを構成します。ハンバーガー・メニュー ![ハンバーガー・メニュー・アイコン](/images/HamburgerMenuSm.jpg) を開き、**「管理」**を展開し、**「呼び出し可能フロー (Callable flows)」**を選択します ([Sharing message flow processing between App Connect and Integration Bus ![外部リンク・アイコン](../../icons/launch-glyph.svg "外部リンク・アイコン")](https://developer.ibm.com/integration/docs/app-connect/tutorials-for-ibm-app-connect/running-your-ibm-integration-bus-solutions-in-ibm-app-connect-enterprise-beta-plan/sharing-message-flow-processing-between-app-connect-and-integration-bus-app-connect-enterprise-beta) を参照)。
